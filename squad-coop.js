(() => {
  ...
  function updateHeader(){
    ...
    if(versionLabel) versionLabel.textContent = state.snapshot && sharedStoryActive()
      ? `Tiger Strike V10.6 • Story Mission ${Math.max(1, Number(state.storyMissionLevel || 1))}`
    : (state.snapshot ? `Tiger Strike V10.6 • ${selectedOperation().mapLabel}` : "Tiger Strike V10.6 • Bloodroot Passage");
  ...
  function coopHomeHtml(){
    ...
    return `<div class="squadPanel">
      ${equipmentButtonsHtml()}
      <div class="squadHomeHero"><div class="squadKicker">V10.6 Tiger Strike Bloodroot Passage</div><div class="squadMissionName">Find your squad</div><div class="squadDesc">Quick Match can find another Telegram player for the exact Story mission or Special Operation you choose. Living World consequences now continue into Chapter 2 Missions 11–13.</div></div>
  ...
  function sharedLivingWorldHtml(){
    ...
    return `<section class="squadEquipmentPanel"><div class="squadKicker">V10.6 • YOUR SHARED STORY WORLD</div><div class="squadSectionTitle">🌍 Living Story Districts</div><div class="squadDesc">Your Shared Story results persist separately from Solo. Chapter 1 remains active, and Bloodroot Passage now affects Chapter 2 Missions 11–13 through Berserker pressure, trail safety, clinics, lanterns, and humane research support.</div><div class="squadSmall"><b>Latest:</b> ${esc(living.headline)}</div><div class="squadGearList">${cards}</div></section>`;
  }
