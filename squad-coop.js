(() => {
  ...
  function updateHeader(){
    ...
    if(versionLabel) versionLabel.textContent = state.snapshot && sharedStoryActive()
      ? `Tiger Strike V10.7 • Story Mission ${Math.max(1, Number(state.storyMissionLevel || 1))}`
    : (state.snapshot ? `Tiger Strike V10.7 • ${selectedOperation().mapLabel}` : "Tiger Strike V10.7 • Amara Haven");
  ...
  function sharedLivingWorldHtml(){
    ...
    return `<section class="squadEquipmentPanel"><div class="squadKicker">V10.7 • YOUR SHARED STORY WORLD</div><div class="squadSectionTitle">🌍 Living Story Districts</div><div class="squadDesc">Your Shared Story results persist separately from Solo. Bloodroot Passage remains active for Missions 11–13, and Amara Haven now changes Missions 14–17 through hospital support, safer escorts, rescue beacons, and civilian protection.</div><div class="squadSmall"><b>Latest:</b> ${esc(living.headline)}</div><div class="squadGearList">${cards}</div></section>`;
  }
