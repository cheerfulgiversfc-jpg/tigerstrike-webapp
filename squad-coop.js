(() => {
  ...
  function updateHeader(){
    ...
    if(versionLabel) versionLabel.textContent = state.snapshot && sharedStoryActive()
      ? `Tiger Strike V10.5 • Story Mission ${Math.max(1, Number(state.storyMissionLevel || 1))}`
    : (state.snapshot ? `Tiger Strike V10.5 • ${selectedOperation().mapLabel}` : "Tiger Strike V10.5 • Iron Roar");
  ...
  function coopHomeHtml(){
    ...
    return `<div class="squadPanel">
      ${equipmentButtonsHtml()}
      <div class="squadHomeHero"><div class="squadKicker">V10.5 Tiger Strike Iron Roar</div><div class="squadMissionName">Find your squad</div><div class="squadDesc">Quick Match can find another Telegram player for the exact Story mission or Special Operation you choose. Every Chapter 1 district now remembers squad choices and changes its Story deployments.</div></div>
  ...
  function sharedLivingWorldHtml(){
    ...
    return `<section class="squadEquipmentPanel"><div class="squadKicker">V10.5 • YOUR SHARED STORY WORLD</div><div class="squadSectionTitle">🌍 Living Chapter 1</div><div class="squadDesc">Your Shared Story results persist separately from Solo. River Gate affects Missions 1–3, Jungle Spine affects Missions 4–7, and Iron Roar affects Missions 8–10 with Armored patrols, industrial power, fortified routes, and Armory support.</div><div class="squadSmall"><b>Latest:</b> ${esc(living.headline)}</div><div class="squadGearList">${cards}</div></section>`;
  }
