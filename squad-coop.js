(() => {
  ...
  const SHARED_STORY_LEVELS = Object.freeze([
    ...
    { level:51, title:"Mountain Village Escort", description:"Escort six villagers through three shared ridge checkpoints." },
    { level:52, title:"Cliffside Attack", description:"Clear seven tigers attacking from the mountain ledges." },
    { level:53, title:"Silverpeak Capture", description:"Use Rubber ammunition to capture the rare Silverpeak Tiger alive." },
    { level:54, title:"Climber Rescue", description:"Rescue four trapped climbers and guide them down three descent checkpoints." },
    { level:55, title:"Mountain Road Pack", description:"Break a nine-tiger pack and secure the mountain road in order." },
    { level:56, title:"Canyon Caravan", description:"Escort five caravan members through three narrow canyon checkpoints." },
    { level:57, title:"Whiteout Patrol", description:"Find seven hidden tigers and activate three beacons in reduced visibility." },
    ...
    { level:71, title:"Jungle Center Evacuation", description:"Escort eight fleeing villagers through three evacuation checkpoints and clear the pursuing pack." },
    { level:72, title:"All-Directions Rescue Ambush", description:"Rescue five surrounded civilians and clear the focused six-tiger ambush." },
    { level:73, title:"Elite Hunter Capture", description:"Capture Razorclaw and Shadowfang alive with Rubber ammunition, then clear their guards." },
    { level:74, title:"Dangerous Jungle Caravan", description:"Escort eight caravan members through three dangerous shared route checkpoints." },
    { level:75, title:"Tiger King Swarm", description:"Survive and clear fifteen coordinated tigers attacking from the Tiger King's territory." },
    { level:76, title:"Lost Soldier Rescue", description:"Find six lost soldiers and guide them through three rally points while clearing their hunters." },
    { level:77, title:"Final Villager Evacuation", description:"Escort ten final villagers through three shared evacuation checkpoints." },
  ...
  function updateHeader(){
    ...
    if(versionLabel) versionLabel.textContent = state.snapshot && sharedStoryActive()
      ? `Tiger Strike V10.9 • Story Mission ${Math.max(1, Number(state.storyMissionLevel || 1))}`
    : (state.snapshot ? `Tiger Strike V10.9 • ${selectedOperation().mapLabel}` : "Tiger Strike V10.9 • Veil Canopy");
  ...
  function coopHomeHtml(){
    ...
    return `<div class="squadPanel">
      ${equipmentButtonsHtml()}
      <div class="squadHomeHero"><div class="squadKicker">V10.9 Tiger Strike Veil Canopy</div><div class="squadMissionName">Find your squad</div><div class="squadDesc">Quick Match can find another Telegram player for the exact Story mission or Special Operation you choose. Living World consequences now continue through Story Mission 23.</div></div>
  ...
  function sharedLivingWorldHtml(){
    ...
    return `<section class="squadEquipmentPanel"><div class="squadKicker">V10.9 • YOUR SHARED STORY WORLD</div><div class="squadSectionTitle">🌍 Living Story Districts</div><div class="squadDesc">Your Shared Story results persist separately from Solo. Veil Canopy extends lasting consequences through Missions 21–23 with exact encounter counts, research-team protection, canopy tracking, safe routes, and humane Veil Tiger capture supplies.</div><div class="squadSmall"><b>Latest:</b> ${esc(living.headline)}</div><div class="squadGearList">${cards}</div></section>`;
  }
  ...
  function drawRescueHelicopter(ctx,x,y,size=1){
    ctx.save();ctx.translate(x,y);ctx.scale(size,size);
    ctx.fillStyle="rgba(2,6,23,.34)";ctx.beginPath();ctx.ellipse(0,34,86,19,0,0,Math.PI*2);ctx.fill();
    ctx.fillStyle="#475569";ctx.strokeStyle="#bae6fd";ctx.lineWidth=4;ctx.beginPath();ctx.ellipse(0,0,55,31,0,0,Math.PI*2);ctx.fill();ctx.stroke();
    ctx.fillStyle="#38bdf8";ctx.beginPath();ctx.ellipse(25,-7,20,16,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#e0f2fe";ctx.lineWidth=3;ctx.stroke();
    ctx.fillStyle="#334155";ctx.fillRect(-88,-7,48,14);ctx.beginPath();ctx.moveTo(-86,-7);ctx.lineTo(-105,-31);ctx.lineTo(-76,-7);ctx.closePath();ctx.fill();
    ctx.strokeStyle="#e2e8f0";ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(-92,-34);ctx.lineTo(-92,27);ctx.moveTo(-117,-5);ctx.lineTo(-67,-5);ctx.stroke();
    ctx.strokeStyle="#cbd5e1";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-84,-43);ctx.lineTo(84,-43);ctx.moveTo(0,-47);ctx.lineTo(0,35);ctx.stroke();
    ctx.fillStyle="#0f172a";ctx.fillRect(-40,28,82,7);ctx.fillStyle="#e0f2fe";ctx.font="950 11px system-ui";ctx.textAlign="center";ctx.fillText("MOUNTAIN RESCUE",0,57);ctx.restore();
  }
  ...
  function drawExpandedDistrict(ctx, snap, view){
    ...
    }else if(storyLevel === 51){
      for(const [xp,yp,size] of [[.18,.23,1.15],[.42,.35,1.25],[.68,.25,1.2],[.82,.57,1.3]]){const x=worldW*xp,y=worldH*yp;if(visible(x,y,180))drawHighlandPeak(ctx,x,y,size);}if(visible(worldW*.55,worldH*.48,380)){ctx.fillStyle="rgba(30,41,59,.95)";roundRect(ctx,worldW*.55-138,worldH*.48-65,276,46,12);ctx.fill();ctx.fillStyle="#f1f5f9";ctx.font="950 17px system-ui";ctx.textAlign="center";ctx.fillText("MOUNTAIN VILLAGE ROUTE",worldW*.55,worldH*.48-35);}
    }else if(storyLevel === 52){
      for(const [xp,yp,size] of [[.14,.20,1.5],[.29,.62,1.35],[.48,.18,1.55],[.67,.63,1.45],[.84,.22,1.6]]){const x=worldW*xp,y=worldH*yp;if(visible(x,y,210))drawHighlandPeak(ctx,x,y,size);}if(visible(worldW*.54,worldH*.48,420)){ctx.fillStyle="rgba(51,65,85,.96)";roundRect(ctx,worldW*.54-132,worldH*.48-70,264,48,12);ctx.fill();ctx.strokeStyle="#fca5a5";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#fee2e2";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("CLIFFSIDE ATTACK",worldW*.54,worldH*.48-39);}
    }else if(storyLevel === 53){
      const tiger=(snap.tigers||[]).find((row)=>row.id==="s53_silverpeak")||{x:worldW*.56,y:worldH*.50};if(visible(tiger.x,tiger.y,370)){drawHighlandPeak(ctx,tiger.x,tiger.y+90,1.4);drawResearchBeacon(ctx,tiger.x,tiger.y,1.35);ctx.fillStyle="rgba(30,41,59,.96)";roundRect(ctx,tiger.x-145,tiger.y-165,290,48,12);ctx.fill();ctx.strokeStyle="#c4b5fd";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#ede9fe";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("SILVERPEAK • LIVE CAPTURE",tiger.x,tiger.y-133);}
    }else if(storyLevel === 54){
      const route=snap.checkpoints||[];if(route.length>1){ctx.strokeStyle="#fde68a";ctx.lineWidth=8;ctx.setLineDash([18,12]);ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);for(const point of route.slice(1))ctx.lineTo(point.x,point.y);ctx.stroke();ctx.setLineDash([]);}for(const [xp,yp,size] of [[.24,.25,1.25],[.56,.35,1.4],[.80,.56,1.2]]){if(visible(worldW*xp,worldH*yp,190))drawHighlandPeak(ctx,worldW*xp,worldH*yp,size);}if(visible(worldW*.55,worldH*.49,390)){ctx.fillStyle="rgba(120,53,15,.95)";roundRect(ctx,worldW*.55-125,worldH*.49-65,250,46,12);ctx.fill();ctx.fillStyle="#fef3c7";ctx.font="950 17px system-ui";ctx.textAlign="center";ctx.fillText("CLIMBER DESCENT ROUTE",worldW*.55,worldH*.49-35);}
    }else if(storyLevel === 55){
      for(const [xp,yp,size] of [[.20,.20,1.2],[.48,.28,1.35],[.74,.19,1.25],[.84,.64,1.3]]){if(visible(worldW*xp,worldH*yp,190))drawHighlandPeak(ctx,worldW*xp,worldH*yp,size);}for(const checkpoint of (snap.checkpoints||[])){if(visible(checkpoint.x,checkpoint.y,130))drawVillageBarricade(ctx,checkpoint.x,checkpoint.y,0);}if(visible(worldW*.54,worldH*.50,400)){ctx.fillStyle="rgba(69,10,10,.95)";roundRect(ctx,worldW*.54-140,worldH*.50-70,280,48,12);ctx.fill();ctx.fillStyle="#ffe4e6";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("MOUNTAIN ROAD PACK",worldW*.54,worldH*.50-39);}
    }else if(storyLevel === 56){
      for(let i=0;i<7;i++){const y=worldH*(.15+i*.10);if(visible(worldW*.24,y,190))drawHighlandPeak(ctx,worldW*.24,y,1.25+(i%2)*.12);if(visible(worldW*.76,y,190))drawHighlandPeak(ctx,worldW*.76,y,1.25+((i+1)%2)*.12);}const points=snap.checkpoints||[],done=snap.checkpointCompletedIds||[],index=Math.min(done.length,Math.max(0,points.length-1)),caravan=points[index]||{x:worldW*.5,y:worldH*.42};if(visible(caravan.x,caravan.y,170))drawConvoyTruck(ctx,caravan.x,caravan.y,0,"#b45309",false);if(visible(worldW*.50,worldH*.50,380)){ctx.fillStyle="rgba(69,26,3,.95)";roundRect(ctx,worldW*.50-125,worldH*.50-70,250,46,12);ctx.fill();ctx.fillStyle="#ffedd5";ctx.font="950 17px system-ui";ctx.textAlign="center";ctx.fillText("CANYON CARAVAN",worldW*.50,worldH*.50-40);}
    }else if(storyLevel === 57){
      for(const checkpoint of (snap.checkpoints||[])){if(!visible(checkpoint.x,checkpoint.y,170))continue;ctx.strokeStyle="#fef08a";ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(checkpoint.x,checkpoint.y+35);ctx.lineTo(checkpoint.x,checkpoint.y-52);ctx.stroke();ctx.fillStyle="#facc15";ctx.beginPath();ctx.arc(checkpoint.x,checkpoint.y-58,13,0,Math.PI*2);ctx.fill();ctx.shadowColor="#fde047";ctx.shadowBlur=32;ctx.beginPath();ctx.arc(checkpoint.x,checkpoint.y-58,8,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}if(visible(worldW*.54,worldH*.48,390)){ctx.fillStyle="rgba(51,65,85,.96)";roundRect(ctx,worldW*.54-135,worldH*.48-70,270,48,12);ctx.fill();ctx.fillStyle="#f8fafc";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("WHITEOUT • FOLLOW BEACONS",worldW*.54,worldH*.48-39);}
    }else if(storyLevel === 58){
      const ex=snap.extraction||{x:worldW*.88,y:worldH*.76};if(visible(ex.x,ex.y,340)){drawRescueHelicopter(ctx,ex.x-125,ex.y-90,1.12);drawRescueHelicopter(ctx,ex.x+120,ex.y-58,.92);}if(visible(worldW*.56,worldH*.50,390)){ctx.fillStyle="rgba(8,47,73,.96)";roundRect(ctx,worldW*.56-140,worldH*.50-72,280,48,12);ctx.fill();ctx.fillStyle="#e0f2fe";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("PROTECT MOUNTAIN RESCUE LZ",worldW*.56,worldH*.50-41);}
    }else if(storyLevel === 59){
    ...
    }else if(storyLevel === 71){
      const route=snap.checkpoints||[];if(route.length>1){ctx.strokeStyle="#facc15";ctx.lineWidth=10;ctx.setLineDash([22,14]);ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);for(const point of route.slice(1))ctx.lineTo(point.x,point.y);ctx.stroke();ctx.setLineDash([]);}for(const point of route){if(visible(point.x,point.y,175))drawVillageBarricade(ctx,point.x,point.y,0);}if(visible(worldW*.54,worldH*.48,400)){ctx.fillStyle="rgba(69,26,3,.96)";roundRect(ctx,worldW*.54-160,worldH*.48-72,320,50,12);ctx.fill();ctx.strokeStyle="#fde047";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#fef9c3";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("JUNGLE CENTER EVACUATION",worldW*.54,worldH*.48-40);}
    }else if(storyLevel === 72){
      const cx=worldW*.53,cy=worldH*.49;for(const [dx,dy] of [[-290,0],[290,0],[0,-220],[0,220],[-205,-160],[205,160]]){if(visible(cx+dx,cy+dy,120))drawBloodTrailMarker(ctx,cx+dx,cy+dy,1.05,"AMBUSH");}if(visible(cx,cy,430)){ctx.strokeStyle="#fb7185";ctx.lineWidth=10;ctx.setLineDash([22,14]);ctx.beginPath();ctx.arc(cx,cy,330,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle="rgba(69,10,10,.97)";roundRect(ctx,cx-174,cy-74,348,50,12);ctx.fill();ctx.fillStyle="#ffe4e6";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("ALL-DIRECTIONS RESCUE AMBUSH",cx,cy-42);}
    }else if(storyLevel === 73){
      for(const tiger of (snap.tigers||[]).filter((row)=>row.id==="s73_razorclaw"||row.id==="s73_shadowfang")){if(visible(tiger.x,tiger.y,200))drawResearchBeacon(ctx,tiger.x,tiger.y,1.22);}if(visible(worldW*.54,worldH*.48,410)){ctx.fillStyle="rgba(49,46,129,.96)";roundRect(ctx,worldW*.54-158,worldH*.48-74,316,50,12);ctx.fill();ctx.strokeStyle="#a5b4fc";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#eef2ff";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("CAPTURE BOTH ELITE HUNTERS",worldW*.54,worldH*.48-42);}
    }else if(storyLevel === 74){
      const points=snap.checkpoints||[],done=snap.checkpointCompletedIds||[],index=Math.min(done.length,Math.max(0,points.length-1)),caravan=points[index]||{x:worldW*.32,y:worldH*.44};if(points.length>1){ctx.strokeStyle="#fbbf24";ctx.lineWidth=9;ctx.setLineDash([24,15]);ctx.beginPath();ctx.moveTo(points[0].x,points[0].y);for(const point of points.slice(1))ctx.lineTo(point.x,point.y);ctx.stroke();ctx.setLineDash([]);}if(visible(caravan.x,caravan.y,180))drawConvoyTruck(ctx,caravan.x,caravan.y,0,"#b45309",false);if(visible(worldW*.54,worldH*.49,410)){ctx.fillStyle="rgba(69,26,3,.96)";roundRect(ctx,worldW*.54-154,worldH*.49-74,308,50,12);ctx.fill();ctx.fillStyle="#ffedd5";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("DANGEROUS JUNGLE CARAVAN",worldW*.54,worldH*.49-42);}
    }else if(storyLevel === 75){
      for(const [xp,yp,r] of [[.18,.25,185],[.40,.48,245],[.65,.28,200],[.80,.66,230]]){const x=worldW*xp,y=worldH*yp;if(!visible(x,y,r+70))continue;ctx.fillStyle="rgba(146,64,14,.20)";ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#f97316";ctx.lineWidth=9;ctx.setLineDash([22,14]);ctx.stroke();ctx.setLineDash([]);}if(visible(worldW*.54,worldH*.50,430)){ctx.fillStyle="rgba(67,20,7,.97)";roundRect(ctx,worldW*.54-160,worldH*.50-76,320,50,12);ctx.fill();ctx.fillStyle="#ffedd5";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("TIGER KING SWARM • 15",worldW*.54,worldH*.50-44);}
    }else if(storyLevel === 76){
      const route=snap.checkpoints||[];if(route.length>1){ctx.strokeStyle="#60a5fa";ctx.lineWidth=9;ctx.setLineDash([20,13]);ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);for(const point of route.slice(1))ctx.lineTo(point.x,point.y);ctx.stroke();ctx.setLineDash([]);}for(const point of route){if(visible(point.x,point.y,180))drawLastStandFort(ctx,point.x,point.y,1.0);}if(visible(worldW*.54,worldH*.48,400)){ctx.fillStyle="rgba(30,58,138,.96)";roundRect(ctx,worldW*.54-144,worldH*.48-72,288,50,12);ctx.fill();ctx.strokeStyle="#93c5fd";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#dbeafe";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("RESCUE THE LOST PATROL",worldW*.54,worldH*.48-40);}
    }else if(storyLevel === 77){
      const route=snap.checkpoints||[];if(route.length>1){ctx.strokeStyle="#4ade80";ctx.lineWidth=11;ctx.setLineDash([24,15]);ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);for(const point of route.slice(1))ctx.lineTo(point.x,point.y);ctx.stroke();ctx.setLineDash([]);}for(const point of route){if(visible(point.x,point.y,180))drawVillageBarricade(ctx,point.x,point.y,0);}if(visible(worldW*.54,worldH*.49,410)){ctx.fillStyle="rgba(20,83,45,.97)";roundRect(ctx,worldW*.54-160,worldH*.49-74,320,50,12);ctx.fill();ctx.strokeStyle="#86efac";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#dcfce7";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("FINAL VILLAGER EVACUATION",worldW*.54,worldH*.49-42);}
    }else if(storyLevel === 78){
      const cx=worldW*.54,cy=worldH*.50;for(const [xp,yp,size] of [[.16,.22,1.0],[.31,.68,1.12],[.55,.25,1.0],[.76,.65,1.18],[.86,.31,1.05]]){const x=worldW*xp,y=worldH*yp;if(visible(x,y,130))drawBloodTrailMarker(ctx,x,y,size,"KING");}if(visible(cx,cy,460)){ctx.fillStyle="rgba(127,29,29,.30)";ctx.beginPath();ctx.arc(cx,cy,360,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#ef4444";ctx.lineWidth=11;ctx.setLineDash([25,15]);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle="rgba(69,10,10,.98)";roundRect(ctx,cx-174,cy-80,348,52,12);ctx.fill();ctx.fillStyle="#ffe4e6";ctx.font="950 18px system-ui";ctx.textAlign="center";ctx.fillText("MASSIVE AMBUSH • 16 TIGERS",cx,cy-47);}
    }else if(storyLevel === 79){
    ...
    if(consequence?.districtId==="iron_roar"){
      const ix=worldW*.55,iy=worldH*.48,powered=!!consequence.support?.powerGrid,fortified=!!consequence.support?.fortifiedGate;
      if(visible(ix,iy,280)){ctx.fillStyle=powered?"rgba(250,204,21,.18)":"rgba(239,68,68,.16)";ctx.strokeStyle=powered?"#fde047":"#fb7185";ctx.lineWidth=7;ctx.beginPath();ctx.arc(ix,iy,112,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle=powered?"#ca8a04":"#475569";roundRect(ctx,ix-58,iy-48,116,96,12);ctx.fill();ctx.strokeStyle="#e2e8f0";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#0f172a";for(let n=-32;n<=32;n+=32)ctx.fillRect(ix+n-8,iy-23,16,46);ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,ix-130,iy-102,260,38,10);ctx.fill();ctx.fillStyle=powered?"#fef9c3":"#ffe4e6";ctx.font="950 13px system-ui";ctx.textAlign="center";ctx.fillText(powered?"IRON ROAR POWER GRID • ONLINE":"IRON ROAR BLACKOUT • ACTIVATE POWER",ix,iy-76);ctx.fillStyle=fortified?"#94a3b8":"#7f1d1d";ctx.fillRect(ix+150,iy-105,24,210);ctx.fillRect(ix+235,iy-105,24,210);ctx.strokeStyle=fortified?"#fbbf24":"#fb7185";ctx.lineWidth=8;ctx.setLineDash([18,10]);ctx.beginPath();ctx.moveTo(ix+174,iy);ctx.lineTo(ix+235,iy);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,ix+128,iy-148,154,30,9);ctx.fill();ctx.fillStyle=fortified?"#dcfce7":"#ffe4e6";ctx.font="950 10px system-ui";ctx.fillText(fortified?"RAIL GATE FORTIFIED":"RAIL GATE BREACHED",ix+205,iy-128);}
      if(consequence.support?.safeRoute&&snap.settlementSupport&&snap.extraction){ctx.strokeStyle="rgba(251,191,36,.72)";ctx.lineWidth=10;ctx.setLineDash([26,14]);ctx.beginPath();ctx.moveTo(snap.settlementSupport.x,snap.settlementSupport.y);ctx.lineTo(ix,iy);ctx.lineTo(snap.extraction.x,snap.extraction.y);ctx.stroke();ctx.setLineDash([]);}
    }
    if(consequence?.districtId==="bloodroot_passage"){
      const px=worldW*.54,py=worldH*.50,lit=!!consequence.support?.lanternNetwork,cleared=!!consequence.support?.safeRoute;
      ctx.strokeStyle=cleared?"rgba(196,181,253,.78)":"rgba(127,29,29,.64)";ctx.lineWidth=cleared?14:8;ctx.setLineDash(cleared?[24,14]:[10,18]);ctx.beginPath();ctx.moveTo(worldW*.17,worldH*.72);ctx.quadraticCurveTo(px,py-150,worldW*.86,worldH*.28);ctx.stroke();ctx.setLineDash([]);
      for(let i=0;i<4;i++){const lx=worldW*(.28+i*.15),ly=worldH*(.66-i*.10);if(!visible(lx,ly,100))continue;ctx.strokeStyle="#78350f";ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(lx,ly);ctx.lineTo(lx,ly-52);ctx.stroke();ctx.fillStyle=lit?"#fde047":"#475569";ctx.shadowColor=lit?"#fbbf24":"transparent";ctx.shadowBlur=lit?24:0;ctx.beginPath();ctx.arc(lx,ly-60,14,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}
      if(visible(px,py,260)){ctx.fillStyle="rgba(69,10,10,.92)";roundRect(ctx,px-150,py-70,300,44,12);ctx.fill();ctx.strokeStyle=lit?"#fde68a":"#fb7185";ctx.lineWidth=3;ctx.stroke();ctx.fillStyle=lit?"#fef3c7":"#ffe4e6";ctx.font="950 14px system-ui";ctx.textAlign="center";ctx.fillText(`${lit?"LANTERNS ACTIVE":"DARK TRAIL"} • ${cleared?"PATH CLEARED":"PATH OVERGROWN"}`,px,py-42);}
    }
    if(consequence?.districtId==="amara_haven"){
      const hx=worldW*.55,hy=worldH*.48,beacons=!!consequence.support?.rescueBeacons,route=!!consequence.support?.caravanRoute,shelter=!!consequence.support?.childShelter;
      ctx.strokeStyle=route?"rgba(56,189,248,.78)":"rgba(248,113,113,.58)";ctx.lineWidth=route?14:8;ctx.setLineDash(route?[28,14]:[9,18]);ctx.beginPath();ctx.moveTo(worldW*.16,worldH*.72);ctx.quadraticCurveTo(hx,hy+120,worldW*.88,worldH*.32);ctx.stroke();ctx.setLineDash([]);
      for(let i=0;i<4;i++){const bx=worldW*(.25+i*.17),by=worldH*(.68-i*.09);if(!visible(bx,by,100))continue;ctx.strokeStyle="#475569";ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(bx,by);ctx.lineTo(bx,by-52);ctx.stroke();ctx.fillStyle=beacons?"#38bdf8":"#475569";ctx.shadowColor=beacons?"#0ea5e9":"transparent";ctx.shadowBlur=beacons?24:0;ctx.beginPath();ctx.arc(bx,by-60,14,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}
      if(visible(hx,hy,260)){ctx.fillStyle="rgba(7,36,58,.94)";roundRect(ctx,hx-168,hy-70,336,44,12);ctx.fill();ctx.strokeStyle=beacons?"#7dd3fc":"#fb7185";ctx.lineWidth=3;ctx.stroke();ctx.fillStyle="#e0f2fe";ctx.font="950 13px system-ui";ctx.textAlign="center";ctx.fillText(`${beacons?"RESCUE BEACONS ACTIVE":"BEACONS OFFLINE"} • ${shelter?"CHILD SHELTER OPEN":"SHELTER CLOSED"}`,hx,hy-42);}
    }
    if(consequence?.districtId==="crimson_hollow"){
      const cx=worldW*.55,cy=worldH*.49,calm=!!consequence.support?.calmingTowers,defense=!!consequence.support?.swarmDefenses,ward=!!consequence.support?.bossWard;
      ctx.fillStyle="rgba(127,29,29,.10)";ctx.strokeStyle=defense?"rgba(251,113,133,.82)":"rgba(127,29,29,.62)";ctx.lineWidth=defense?10:6;ctx.setLineDash(defense?[26,13]:[10,18]);ctx.beginPath();ctx.moveTo(worldW*.22,worldH*.30);ctx.lineTo(worldW*.80,worldH*.26);ctx.lineTo(worldW*.88,worldH*.73);ctx.lineTo(worldW*.27,worldH*.78);ctx.closePath();ctx.fill();ctx.stroke();ctx.setLineDash([]);
      for(let i=0;i<3;i++){const tx=worldW*(.32+i*.22),ty=worldH*(.36+i*.12);if(!visible(tx,ty,100))continue;ctx.fillStyle=calm?"#facc15":"#475569";ctx.shadowColor=calm?"#f59e0b":"transparent";ctx.shadowBlur=calm?22:0;ctx.fillRect(tx-9,ty-54,18,54);ctx.beginPath();ctx.arc(tx,ty-62,16,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}
      if(visible(cx,cy,280)){ctx.fillStyle="rgba(69,10,10,.94)";roundRect(ctx,cx-184,cy-72,368,46,12);ctx.fill();ctx.strokeStyle=ward?"#facc15":"#fb7185";ctx.lineWidth=3;ctx.stroke();ctx.fillStyle=ward?"#fef3c7":"#fee2e2";ctx.font="950 13px system-ui";ctx.textAlign="center";ctx.fillText(`${calm?"CALMING TOWERS ACTIVE":"TOWERS OFFLINE"} • ${ward?"BLOOD TIGER WARD ACTIVE":"RAGE WARD OFFLINE"}`,cx,cy-43);}
    }
    if(consequence?.districtId==="veil_canopy"){
      const vx=worldW*.55,vy=worldH*.49,beacons=!!consequence.support?.canopyBeacons,route=!!consequence.support?.protectedResearchRoute,lab=!!consequence.support?.veilLab;
    ...
    const support=snap.settlementSupport;
    if(support&&support.type==="iron_armory"&&visible(support.x,support.y,210)){ctx.fillStyle="rgba(245,158,11,.18)";ctx.strokeStyle="#fbbf24";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#64748b";roundRect(ctx,support.x-70,support.y-46,140,92,8);ctx.fill();ctx.strokeStyle="#cbd5e1";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#1e293b";for(let n=-42;n<=42;n+=28)ctx.fillRect(support.x+n-7,support.y-30,14,60);ctx.fillStyle="#f59e0b";ctx.fillRect(support.x-17,support.y+4,34,42);ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,support.x-110,support.y-118,220,32,9);ctx.fill();ctx.fillStyle="#fef3c7";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"IRON ROAR ARMORY").toUpperCase(),support.x,support.y-96);for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+75;ctx.fillStyle=i?"#94a3b8":"#f59e0b";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#fef3c7";ctx.font="900 9px system-ui";ctx.fillText(i?"ENGINEER":"ARMORER",wx,wy+29);}}
    if(support&&support.type==="bloodroot_clinic"&&visible(support.x,support.y,210)){ctx.fillStyle="rgba(190,24,93,.17)";ctx.strokeStyle="#f9a8d4";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#f8fafc";ctx.beginPath();ctx.moveTo(support.x-76,support.y+42);ctx.lineTo(support.x,support.y-66);ctx.lineTo(support.x+76,support.y+42);ctx.closePath();ctx.fill();ctx.strokeStyle="#f9a8d4";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#be123c";ctx.fillRect(support.x-9,support.y-36,18,58);ctx.fillRect(support.x-29,support.y-16,58,18);ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,support.x-116,support.y-118,232,32,9);ctx.fill();ctx.fillStyle="#fce7f3";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"BLOODROOT CLINIC").toUpperCase(),support.x,support.y-96);for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+76;ctx.fillStyle=i?"#a78bfa":"#fb7185";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#fce7f3";ctx.font="900 9px system-ui";ctx.fillText(i?"RESEARCHER":"MEDIC",wx,wy+29);}}
    if(support&&support.type==="amara_hospital"&&visible(support.x,support.y,220)){ctx.fillStyle="rgba(14,165,233,.18)";ctx.strokeStyle="#7dd3fc";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#e0f2fe";roundRect(ctx,support.x-72,support.y-48,144,96,10);ctx.fill();ctx.strokeStyle="#38bdf8";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#e11d48";ctx.fillRect(support.x-9,support.y-34,18,58);ctx.fillRect(support.x-29,support.y-14,58,18);ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,support.x-122,support.y-122,244,32,9);ctx.fill();ctx.fillStyle="#e0f2fe";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"AMARA HAVEN HOSPITAL").toUpperCase(),support.x,support.y-100);for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+78;ctx.fillStyle=i?"#f472b6":"#38bdf8";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#e0f2fe";ctx.font="900 9px system-ui";ctx.fillText(i?"DRIVER":"NURSE",wx,wy+29);}}
    if(support&&support.type==="crimson_camp"&&visible(support.x,support.y,220)){ctx.fillStyle="rgba(127,29,29,.20)";ctx.strokeStyle="#fb7185";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#450a0a";roundRect(ctx,support.x-75,support.y-48,150,96,10);ctx.fill();ctx.strokeStyle="#facc15";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#fee2e2";ctx.fillRect(support.x-8,support.y-32,16,58);ctx.fillRect(support.x-29,support.y-11,58,16);ctx.strokeStyle="#facc15";ctx.beginPath();ctx.arc(support.x,support.y-3,35,0,Math.PI*2);ctx.stroke();ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,support.x-132,support.y-122,264,32,9);ctx.fill();ctx.fillStyle="#fee2e2";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"CRIMSON CONSERVATION CAMP").toUpperCase(),support.x,support.y-100);for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+78;ctx.fillStyle=i?"#facc15":"#fb7185";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#fee2e2";ctx.font="900 9px system-ui";ctx.fillText(i?"TRACKER":"HANDLER",wx,wy+29);}}
    if(support&&support.type==="veil_outpost"&&visible(support.x,support.y,220)){ctx.fillStyle="rgba(5,150,105,.18)";ctx.strokeStyle="#6ee7b7";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#064e3b";roundRect(ctx,support.x-76,support.y-48,152,96,10);ctx.fill();ctx.strokeStyle="#67e8f9";ctx.lineWidth=4;ctx.stroke();ctx.fillStyle="#d1fae5";ctx.fillRect(support.x-8,support.y-33,16,59);ctx.fillRect(support.x-30,support.y-12,60,17);ctx.fillStyle="#22d3ee";ctx.beginPath();ctx.arc(support.x+47,support.y-28,9,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(2,6,23,.94)";roundRect(ctx,support.x-128,support.y-122,256,32,9);ctx.fill();ctx.fillStyle="#d1fae5";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"VEIL CANOPY OUTPOST").toUpperCase(),support.x,support.y-100);for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+78;ctx.fillStyle=i?"#22d3ee":"#34d399";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#d1fae5";ctx.font="900 9px system-ui";ctx.fillText(i?"BIOLOGIST":"RANGER",wx,wy+29);}}
    if(support&&!['iron_armory','bloodroot_clinic','amara_hospital','crimson_camp','veil_outpost'].includes(support.type)&&visible(support.x,support.y,190)){ctx.fillStyle="rgba(16,185,129,.18)";ctx.strokeStyle="#6ee7b7";ctx.lineWidth=6;ctx.beginPath();ctx.arc(support.x,support.y,support.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#d1fae5";ctx.fillRect(support.x-52,support.y-34,104,72);ctx.fillStyle="#065f46";ctx.beginPath();ctx.moveTo(support.x-66,support.y-34);ctx.lineTo(support.x,support.y-82);ctx.lineTo(support.x+66,support.y-34);ctx.closePath();ctx.fill();ctx.fillStyle="#047857";ctx.fillRect(support.x-12,support.y+2,24,36);ctx.fillStyle="rgba(2,44,34,.94)";roundRect(ctx,support.x-104,support.y-118,208,30,9);ctx.fill();ctx.fillStyle="#d1fae5";ctx.font="950 12px system-ui";ctx.textAlign="center";ctx.fillText(String(support.label||"DISTRICT SUPPORT").toUpperCase(),support.x,support.y-98);if(support.type==="jungle_ranger"){for(let i=0;i<Math.max(1,Number(support.returningCivilians||0));i++){const wx=support.x-30+i*60,wy=support.y+70;ctx.fillStyle="#86efac";ctx.beginPath();ctx.arc(wx,wy-18,9,0,Math.PI*2);ctx.fill();ctx.fillStyle="#166534";ctx.fillRect(wx-8,wy-9,16,25);ctx.fillStyle="#d1fae5";ctx.font="900 9px system-ui";ctx.fillText(i?"SCOUT":"MEDIC",wx,wy+29);}}}
  ...
  function drawSnowstormOverlay(ctx,snap,view,now){
    const intensity=clamp(Number(snap.mission?.snowstormIntensity||0),0,.9);if(intensity<=0)return;
    ctx.save();ctx.fillStyle=`rgba(226,232,240,${.18+intensity*.42})`;ctx.fillRect(view.x,view.y,view.w,view.h);
    ctx.strokeStyle=`rgba(255,255,255,${.42+intensity*.48})`;ctx.lineWidth=2+intensity*3;ctx.lineCap="round";
    const count=Math.round(45+intensity*95),travel=(now*.18)%(view.w+180);
    for(let i=0;i<count;i++){const seed=i*97.31,x=view.x+((i*137+travel)%(view.w+180))-90,y=view.y+((i*83+seed)%(view.h+120))-60,len=12+(i%7)*5;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+len,y+len*.38);ctx.stroke();}
