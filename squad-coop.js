(() => {
  ...
  function updateHeader(){
    ...
    if(versionLabel) versionLabel.textContent = state.snapshot && sharedStoryActive()
      ? `Tiger Strike V10.8 • Story Mission ${Math.max(1, Number(state.storyMissionLevel || 1))}`
    : (state.snapshot ? `Tiger Strike V10.8 • ${selectedOperation().mapLabel}` : "Tiger Strike V10.8 • Crimson Hollow");
  ...
  function coopHomeHtml(){
    ...
    return `<div class="squadPanel">
      ${equipmentButtonsHtml()}
      <div class="squadHomeHero"><div class="squadKicker">V10.8 Tiger Strike Crimson Hollow</div><div class="squadMissionName">Find your squad</div><div class="squadDesc">Quick Match can find another Telegram player for the exact Story mission or Special Operation you choose. Every Chapter 2 mission now has persistent Living World consequences.</div></div>
  ...
  function sharedLivingWorldHtml(){
    ...
    return `<section class="squadEquipmentPanel"><div class="squadKicker">V10.8 • YOUR SHARED STORY WORLD</div><div class="squadSectionTitle">🌍 Living Story Districts</div><div class="squadDesc">Your Shared Story results persist separately from Solo. Crimson Hollow completes Chapter 2 across Missions 18–20 with exact encounter counts, humane capture supplies, calming towers, swarm defenses, and Blood Tiger rage protection.</div><div class="squadSmall"><b>Latest:</b> ${esc(living.headline)}</div><div class="squadGearList">${cards}</div></section>`;
  }
