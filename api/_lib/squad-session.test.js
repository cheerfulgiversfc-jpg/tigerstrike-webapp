...
async function run(){
  ...
  for(let level=2; level<=80; level++){
    ...
    if(level === 51){
      assert.equal(levelSnapshot.mission.chapterName, "Mountain Edge", "Mission 51 starts the real Chapter 6 campaign");
      assert.equal(levelSnapshot.mission.rescueRequired, 6, "Mission 51 escorts six mountain villagers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 51 has three shared ridge checkpoints");
    }
    if(level === 52) assert.equal(levelSnapshot.tigers.length, 7, "Mission 52 has seven cliffside attackers");
    if(level === 53){
      assert.equal(levelSnapshot.mission.captureRequired, 1, "Mission 53 requires one rare mountain capture");
      assert.deepEqual(levelSnapshot.mission.captureTargetIds, ["s53_silverpeak"], "Mission 53 requires Silverpeak rather than any tiger");
      assert.deepEqual(levelSnapshot.mission.captureTargetNames, ["Silverpeak Mountain Tiger"], "Mission 53 names its required live-capture target");
    }
    if(level === 54){
      assert.equal(levelSnapshot.mission.rescueRequired, 4, "Mission 54 rescues all four trapped climbers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 54 has three descent checkpoints");
      assert.equal(levelSnapshot.civilians.filter((civilian)=>civilian.injured).length, 1, "Mission 54 marks its injured climber");
    }
    if(level === 55){
      assert.equal(levelSnapshot.tigers.length, 9, "Mission 55 contains the complete nine-tiger mountain-road pack");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 55 secures the mountain road in order");
    }
    if(level === 56){
      assert.equal(levelSnapshot.mission.rescueRequired, 5, "Mission 56 escorts all five caravan members");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 56 advances through three canyon checkpoints");
    }
    if(level === 57){
      assert.equal(levelSnapshot.tigers.length, 7, "Mission 57 hides seven tigers in the whiteout");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 57 has three real visibility beacons");
      assert.equal(levelSnapshot.mission.snowstormIntensity, .78, "Mission 57 exposes its heavy reduced-visibility snowstorm");
    }
    if(level === 58){
      assert.equal(levelSnapshot.mission.rescueRequired, 5, "Mission 58 protects the complete rescue crew");
      assert.equal(levelSnapshot.mission.extractionType, "helicopter", "Mission 58 uses a real helicopter-marked extraction zone");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 58 secures the mountain LZ in three stages");
    }
    if(level === 59){
      assert.equal(levelSnapshot.tigers.length, 12, "Mission 59 contains the full twelve-tiger mountain swarm");
      assert.equal(levelSnapshot.mission.aggressionBonus, 6, "Mission 59 begins at extreme mountain aggression");
    }
    if(level === 60){
      assert.equal(levelSnapshot.boss.name, "Mountain Alpha Tiger", "Mission 60 uses the real Mountain Alpha boss");
      assert.equal(levelSnapshot.boss.hpMax, 3200, "Mission 60 keeps the Mountain Alpha boss health");
      assert.equal(levelSnapshot.boss.bloodRage, true, "Mission 60 carries the low-health Summit Rage phase");
    }
    if(level === 61){
      assert.equal(levelSnapshot.mission.chapterName, "Tiger Territory", "Mission 61 starts the real Chapter 7 campaign");
      assert.equal(levelSnapshot.mission.rescueRequired, 6, "Mission 61 escorts all six scientists");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 61 has three shared research checkpoints");
    }
    if(level === 62){
      assert.equal(levelSnapshot.tigers.length, 8, "Mission 62 has eight cave-entrance guards");
      assert.equal(levelSnapshot.checkpoints.length, 4, "Mission 62 secures four cave entrances in order");
    }
    if(level === 63){
      assert.equal(levelSnapshot.mission.captureRequired, 4, "Mission 63 requires four live research captures");
      assert.deepEqual(levelSnapshot.mission.captureTargetIds, ["s63_sample_ember","s63_sample_vine","s63_sample_mist","s63_sample_dusk"], "Mission 63 requires its four named research tigers");
      assert.equal(levelSnapshot.mission.captureTargetNames.length, 4, "Mission 63 names every required research tiger");
    }
    if(level === 64){
      assert.equal(levelSnapshot.mission.rescueRequired, 7, "Mission 64 escorts all seven tunnel villagers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 64 has a real three-stage cave route");
    }
    if(level === 65) assert.equal(levelSnapshot.tigers.length, 13, "Mission 65 contains the complete thirteen-tiger territory pack");
    if(level === 66){
      assert.equal(levelSnapshot.mission.rescueRequired, 4, "Mission 66 protects all four camp specialists");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 66 secures three temporary-base perimeter posts");
    }
    if(level === 67){
      assert.equal(levelSnapshot.tigers.length, 8, "Mission 67 contains eight stealth tigers");
      assert(levelSnapshot.tigers.every((tiger)=>tiger.type === "Stalker"), "Mission 67 uses an all-Stalker night pack");
      assert.equal(levelSnapshot.mission.nightVisibilityIntensity, .78, "Mission 67 exposes its reduced-visibility night layer");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 67 uses three working night beacons");
    }
    if(level === 68){
      assert.equal(levelSnapshot.mission.rescueRequired, 1, "Mission 68 protects the lead equipment scientist");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 68 has three working research-equipment sites");
    }
    if(level === 69){
      assert.equal(levelSnapshot.tigers.length, 14, "Mission 69 contains the full fourteen-tiger extreme-aggression pack");
      assert.equal(levelSnapshot.mission.aggressionBonus, 8, "Mission 69 starts with extreme aggression damage");
    }
    if(level === 70){
      assert.equal(levelSnapshot.boss.name, "Legendary Blood Tiger", "Mission 70 uses the real Legendary Blood Tiger boss");
      assert.equal(levelSnapshot.boss.hpMax, 3900, "Mission 70 keeps the Legendary Blood Tiger boss health");
      assert.equal(levelSnapshot.boss.bloodRage, true, "Mission 70 carries the low-health Ancient Blood Rage phase");
    }
    if(level === 71){
      assert.equal(levelSnapshot.mission.chapterName, "The Tiger King", "Mission 71 starts the real Chapter 8 campaign");
      assert.equal(levelSnapshot.mission.rescueRequired, 8, "Mission 71 escorts all eight fleeing villagers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 71 has three shared evacuation checkpoints");
    }
    if(level === 72){
      assert.equal(levelSnapshot.mission.rescueRequired, 5, "Mission 72 rescues the intended five civilians");
      assert.equal(levelSnapshot.tigers.length, 6, "Mission 72 uses the corrected focused six-tiger ambush");
      assert(levelSnapshot.mission.dangerNote.includes("no hidden objective"), "Mission 72 explains that no invisible objective remains");
    }
    if(level === 73){
      assert.equal(levelSnapshot.mission.captureRequired, 2, "Mission 73 requires both elite hunter captures");
      assert.deepEqual(levelSnapshot.mission.captureTargetIds, ["s73_razorclaw","s73_shadowfang"], "Mission 73 requires Razorclaw and Shadowfang specifically");
      assert.equal(levelSnapshot.mission.captureTargetNames.length, 2, "Mission 73 names both required elite captures");
    }
    if(level === 74){
      assert.equal(levelSnapshot.mission.rescueRequired, 8, "Mission 74 protects the complete eight-person caravan");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 74 moves the caravan through three shared checkpoints");
    }
    if(level === 75){
      assert.equal(levelSnapshot.tigers.length, 15, "Mission 75 contains the full fifteen-tiger swarm");
      assert.equal(levelSnapshot.mission.aggressionBonus, 8, "Mission 75 begins at Tiger King territory aggression");
    }
    if(level === 76){
      assert.equal(levelSnapshot.mission.rescueRequired, 6, "Mission 76 rescues all six lost soldiers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 76 has three real patrol rally points");
      assert(levelSnapshot.civilians.every((civilian)=>civilian.look === "soldier"), "Mission 76 visibly renders every rescue target as a soldier");
    }
    if(level === 77){
      assert.equal(levelSnapshot.mission.rescueRequired, 10, "Mission 77 escorts all ten final villagers");
      assert.equal(levelSnapshot.checkpoints.length, 3, "Mission 77 has three shared final-evacuation checkpoints");
    }
    if(level === 78){
      assert.equal(levelSnapshot.tigers.length, 16, "Mission 78 contains the complete sixteen-tiger massive ambush");
      assert.equal(levelSnapshot.mission.aggressionBonus, 9, "Mission 78 starts with massive-ambush aggression damage");
    }
    if(level === 79){
      assert.equal(levelSnapshot.checkpoints.length, 4, "Mission 79 turns preparation into four working field stations");
      assert.equal(levelSnapshot.tigers.length, 12, "Mission 79 includes the Tiger King's full twelve-tiger vanguard");
    }
    if(level === 80){
      assert.equal(levelSnapshot.boss.name, "Tiger King", "Mission 80 uses the real Tiger King boss");
    ...
    if(level === 60){
      assert.equal(levelHostReward.reward.cash, 46500, "Mission 60 pays the Mountain Alpha cash reward");
      assert.equal(levelHostReward.reward.badge, "Mountain Alpha Breakers", "Mission 60 awards the Mountain Alpha badge");
      assert.deepEqual(levelHostReward.storyProgress, { completedLevel:60, unlockLevel:61 }, "Mission 60 unlocks Mission 61");
    }
    if(level === 70){
      assert.equal(levelHostReward.reward.cash, 69000, "Mission 70 pays the Legendary Blood Tiger cash reward");
      assert.equal(levelHostReward.reward.badge, "Legendary Blood Tiger Breakers", "Mission 70 awards the Legendary Blood Tiger badge");
      assert.deepEqual(levelHostReward.storyProgress, { completedLevel:70, unlockLevel:71 }, "Mission 70 unlocks Mission 71");
    }
    if(level === 80){
      assert.equal(levelHostReward.reward.cash, 105000, "Mission 80 pays the Tiger King cash reward");
