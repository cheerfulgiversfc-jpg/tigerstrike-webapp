...
const context = {
  ...
  fetch:async()=>({ ok:true, json:async()=>({ ok:true, profile:{ userId:77, unlockedStoryLevel:100 }, gearCatalog:[] }) }),
};
...
async function run(){
  ...
  assert(html.includes("Unlocked 100/100"), "Story path uses the co-op campaign profile instead of Solo progress");
  assert(html.includes("Story Mission 100"), "Story path defaults to the final co-op mission");
  ...
  await clickCommand("select-story", { squadStoryLevel:"91" });
  html = element("squadBody").innerHTML;
  assert(html.includes("Jungle Core Evacuation"), "Mission 91 exposes its final villager evacuation route");
  assert(html.includes("Two Player ready"), "Mission 91 is marked playable with a teammate");

  await clickCommand("select-story", { squadStoryLevel:"93" });
  html = element("squadBody").innerHTML;
  assert(html.includes("Elite Guardian Capture"), "Mission 93 exposes its required Guardian live capture");

  await clickCommand("select-story", { squadStoryLevel:"97" });
  html = element("squadBody").innerHTML;
  assert(html.includes("Final Helicopter Defense"), "Mission 97 exposes its helicopter defense objective");

  await clickCommand("select-story", { squadStoryLevel:"99" });
  html = element("squadBody").innerHTML;
  assert(html.includes("Ancient Temple Entry"), "Mission 99 exposes its working temple seals");

  await clickCommand("select-story", { squadStoryLevel:"100" });
  html = element("squadBody").innerHTML;
  assert(html.includes("The Ancient Tiger"), "Mission 100 exposes its final Ancient Tiger boss objective");
  assert(html.includes("Two Player ready"), "Mission 100 is marked playable with a teammate");
